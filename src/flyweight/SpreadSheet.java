...
public class SpreadSheet {
  ...
  public void setFontFamily(int row, int col, String fontFamily) {
    ...
    var currentFontData = cell.getFontData();
    cell.setFontData(new CellFontData(currentFontData.getSize(), currentFontData.getIsBold(), fontFamily));
  }
  ...
  private void generateCells() {
    for (var row = 0; row < MAX_ROWS; row++)
      for (var col = 0; col < MAX_COLS; col++) {
        var cell = new Cell(row, col, new CellFontData(fontSize, isBold, fontFamily));
        cells[row][col] = cell;
      }
  }
